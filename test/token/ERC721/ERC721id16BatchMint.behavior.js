...
const secondTokenId = new BN('13681');
const nonExistentTokenId = new BN('25013');
...
function shouldBehaveLikeERC721 (errorPrefix, owner, newOwner, approved, anotherApproved, operator, other) {
  ...
  context('with minted tokens', function () {
    beforeEach(async function () {
      const defaultOwner = await this.token.defaultOwner();
      await this.token.transferFrom(defaultOwner, owner, firstTokenId);
      await this.token.transferFrom(defaultOwner, owner, secondTokenId);
  ...
  describe.skip('_mint(address, uint256)', function () {
  ...
  describe('initial transfer', function () {
    ...
    beforeEach(async function () {
      defaultOwner = await this.token.defaultOwner();
    });

    it('cannot transfer token 0', async function () {
      await expectRevert(
        this.token.transferFrom(defaultOwner, owner, 0), 'ERC721: invalid token ID',
      );
    })

    it('cannot transfer token 20001', async function () {
      await expectRevert(
        this.token.transferFrom(defaultOwner, owner, 20001), 'ERC721: invalid token ID',
      );
    })

    it('can transfer token 20000', async function () {
      await this.token.transferFrom(defaultOwner, owner, 20000);
      expect(await this.token.ownerOf(20000)).to.equal(owner);
    })

    it('emits an initial Transfer event', async function () {
      // do not know how to test this event better
      let fakeReceipt = {};
      fakeReceipt.logs = await this.token.getPastEvents('ConsecutiveTransfer');
      expect(fakeReceipt.logs.length).to.equal(1);
      expectEvent(
        fakeReceipt,
        'ConsecutiveTransfer',
        { fromTokenId: '1', toTokenId: '20000', from: ZERO_ADDRESS, to: defaultOwner }
      );
    });
    ...
    it('emits a Transfer event', async function () {
      expectEvent(
        await this.token.transferFrom(defaultOwner, owner, firstTokenId),
        'Transfer',
        { from: defaultOwner, to: owner, tokenId: firstTokenId }
      );
    });

    it('reverts when transfer by not a default owner', async function () {
      await expectRevert(
        this.token.transferFrom(defaultOwner, owner, firstTokenId, {from: other}), 'ERC721: caller is not token owner or approved',
      );
    })

    it('reverts when from is not the owner', async function () {
      await expectRevert(
        this.token.transferFrom(other, owner, firstTokenId), 'ERC721: transfer from incorrect owner',
      );
    })

    it('an operator can transfer', async function () {
      await this.token.setApprovalForAll(operator, true);
      await this.token.transferFrom(defaultOwner, owner, firstTokenId, {from: operator});
      expect(await this.token.ownerOf(firstTokenId)).to.equal(owner);
    })

    it('can approve before an initial transfer', async function () {
      await this.token.approve(approved, firstTokenId)
      expect(await this.token.getApproved(firstTokenId)).to.be.equal(approved);
    })

    it('an approved token can be transferred', async function () {
      await this.token.approve(approved, firstTokenId)
      await this.token.transferFrom(defaultOwner, owner, firstTokenId, {from: approved});
      expect(await this.token.ownerOf(firstTokenId)).to.equal(owner);
    })

    it('a default owner is an initial owner', async function () {
      expect(await this.token.ownerOf(firstTokenId)).to.equal(defaultOwner);
    })

    it('reverts when calling ownerOf() for token ID out of range', async function () {
      await expectRevert(
        this.token.ownerOf(20001), 'ERC721: invalid token ID',
      );
    })
  })
...
function shouldBehaveLikeERC721Enumerable (errorPrefix, owner, newOwner, approved, anotherApproved, operator, other) {
  ...
  describe('tokenOfOwnerByIndex with initial transfers', function () {
    let defaultOwner;
    const indexToCheck = firstTokenId.toNumber() - 1;

    beforeEach(async function () {
      defaultOwner = await this.token.defaultOwner();
    })

    it('initial balanceOf', async function () {
      expect(await this.token.balanceOf(defaultOwner)).to.be.bignumber.equal('20000');
    })

    it('balanceOf after a transfer', async function () {
      await this.token.transferFrom(defaultOwner, owner, firstTokenId);
      expect(await this.token.balanceOf(defaultOwner)).to.be.bignumber.equal('19999');
    })

    it('an initial index', async function () {
      expect(await this.token.tokenOfOwnerByIndex(defaultOwner, indexToCheck)).to.be.bignumber.equal(firstTokenId);
    })

    it('an index after a transfer', async function () {
      await this.token.transferFrom(defaultOwner, owner, firstTokenId);
      expect(await this.token.tokenOfOwnerByIndex(defaultOwner, indexToCheck)).to.be.bignumber.equal('20000');
    })

    it('an index after two transfers', async function () {
      await this.token.transferFrom(defaultOwner, owner, firstTokenId);
      await this.token.transferFrom(defaultOwner, owner, 20000);
      expect(await this.token.tokenOfOwnerByIndex(defaultOwner, indexToCheck)).to.be.bignumber.equal('19999');
    })
  })
  ...
  context('with minted tokens', function () {
    ...
    describe('totalSupply', function () {
      it('returns total token supply', async function () {
        expect(await this.token.totalSupply()).to.be.bignumber.equal('20000');
      });
    });
    ...
    describe('tokenOfOwnerByIndex', function () {
      ...
      describe('when the index is greater than or equal to the total tokens owned by the given address', function () {
        it('reverts', async function () {
          await expectRevert(
            this.token.tokenOfOwnerByIndex(owner, 2), 'List16Lib: getPos A',
          );
        });
      });
      ...
      describe('when the given address does not own any token', function () {
        it('reverts', async function () {
          await expectRevert(
            this.token.tokenOfOwnerByIndex(other, 0), 'List16Lib: getPos A',
          );
        });
      });
      ...
      describe('after transferring all tokens to another user', function () {
        ...
        it('returns empty collection for original owner', async function () {
          ...
          await expectRevert(
            this.token.tokenOfOwnerByIndex(owner, 0), 'List16Lib: getPos A',
          );
        });
      });
    });
    ...
    describe('tokenByIndex', function () {
      it('returns all tokens', async function () {
        const tokensListed = await Promise.all(
          [0, 1, firstTokenId.toNumber() - 1, secondTokenId.toNumber() - 1, 19999].map(i => this.token.tokenByIndex(i)),
        );
        expect(tokensListed.map(t => t.toNumber())).to.have.members([1, 2, firstTokenId.toNumber(),
          secondTokenId.toNumber(), 20000]);
      });

      it('reverts if index is greater than supply', async function () {
        await expectRevert(
          this.token.tokenByIndex(20000), 'ERC721Enumerable: global index out of bounds',
        );
      });

      [firstTokenId, secondTokenId].forEach(function (tokenId) {
        it.skip(`returns all tokens after burning token ${tokenId} and minting new tokens`, async function () {
  ...
  describe('_mint(address, uint256)', function () {
    ...
    context('with minted token', async function () {
      beforeEach(async function () {
        const defaultOwner = await this.token.defaultOwner();
        this.receipt = await this.token.transferFrom(defaultOwner, owner, firstTokenId);
      });
      ...
      it('adjusts all tokens list', async function () {
        expect(await this.token.tokenByIndex(0)).to.be.bignumber.equal("1");
      });
    });
  });
