  ...
  describe('initial transfer', function () {
    ...
    it('emits an initial Transfer event', async function () {
      expectEvent(
        await this.token.transferFrom(defaultOwner, owner, firstTokenId),
        'Transfer',
        { from: ZERO_ADDRESS, to: defaultOwner, tokenId: firstTokenId }
      );
    });
    ...
    it('cannot approve before an initial transfer', async function () {
      await expectRevert(
        this.token.approve(approved, firstTokenId), 'ERC721: invalid token ID',
      );
    })
    ...
    it('can change the default owner', async function () {
      await this.token.transferDefaultOwner(owner);
      expect(await this.token.ownerOf(firstTokenId)).to.equal(owner);
    })

    it('can change the default owner with the event', async function () {
      expectEvent(
        await this.token.transferDefaultOwner(owner),
        'NewDefaultOwner',
        { oldDefaultOwner: defaultOwner, newDefaultOwner: owner }
      );
    })

    it('reverts when a default owner is set to ZERO', async function () {
      await expectRevert(
        this.token.transferDefaultOwner(ZERO_ADDRESS), 'ERC721: non zero default owner',
      );
    })

    it('reverts when a default owner is changed by not a default owner', async function () {
      await expectRevert(
        this.token.transferDefaultOwner(owner, {from: other}), 'ERC721: caller is not default owner'
      );
    })
  })
